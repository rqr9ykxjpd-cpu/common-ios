...
extension AppState {
    func advance(from step: OnboardingStep) {
        ...
        withAnimation(CampusTheme.Motion.easing) { route = .onboarding(next) }
    }
    ...
    func finishOnboarding() async {
        ...
        guard let avatarData else {
            ...
            onboardingFailure = message
            showError(message)
            withAnimation(CampusTheme.Motion.easing) { route = .onboarding(.photo) }
            return
        }
        do {
    ...
    func goBack(from step: OnboardingStep) {
        if let previous = OnboardingStep(rawValue: step.rawValue - 1) {
            withAnimation(CampusTheme.Motion.easing) { route = .onboarding(previous) }
        } else {
            withAnimation(CampusTheme.Motion.easing) { route = .welcome }
        }
    }
}
