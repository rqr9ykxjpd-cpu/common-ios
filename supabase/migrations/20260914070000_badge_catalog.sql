-- Rozet kataloğu: gönderide rozet zorunlu (istemci zorlar), sunucu geçerli
-- değerleri kısıtla tutar. 'moment' eski satırlar ve bilinmeyen değerler için
-- kalır. İstemcideki `PostKind` ile birebir aynı liste.
...
alter table public.posts add constraint posts_kind_check
  check (kind in (
    ...
    'photo', 'life_story', 'instant', 'serious'
  ));
