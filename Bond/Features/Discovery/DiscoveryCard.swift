        ...
        GeometryReader { proxy in
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 24, style: .continuous)
                    .stroke(kurucu ? BondTheme.ember.opacity(0.85) : .white.opacity(0.16),
                            lineWidth: kurucu ? 2 : 1)
            )
            .shadow(color: kurucu ? BondTheme.ember.opacity(0.32) : .clear, radius: 18)
    ...
    private var details: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(profile.bio)
                ...
                .foregroundStyle(BondTheme.ink.opacity(0.72))
                .lineSpacing(2)
                .lineLimit(2)
            ...
            HStack(spacing: 7) {
                ...
                ForEach(sortedInterests.prefix(3), id: \.self) { item in
                    ...
                    .foregroundStyle(shared ? BondTheme.violet : BondTheme.ink)
                    .padding(.horizontal, 9)
                    .frame(height: 26)
                    .background(shared ? BondTheme.violet.opacity(0.12) : BondTheme.ink.opacity(0.055), in: Capsule())
                    .overlay(Capsule().stroke(shared ? BondTheme.violet.opacity(0.35) : .clear))
                }
                Spacer()
            }
