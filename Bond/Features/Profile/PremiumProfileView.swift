        ...
        ZStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 26) {
                    HStack { Wordmark(compact: true); Spacer(); Eyebrow(text: L10n.Profile.premiumEyebrow, color: BondTheme.ink.opacity(0.5)) }
                    ZStack(alignment: .bottomLeading) {
                        LinearGradient(colors: [BondTheme.violet, BondTheme.coral], startPoint: .topLeading, endPoint: .bottomTrailing)
                            .frame(height: 260)
                    ...
                    Text(L10n.Profile.speaksFirst).editorialTitle(37).foregroundStyle(BondTheme.ink)
                    ...
                    Button(L10n.Profile.signOutCaps) { Task { await appState.signOut() } }
                        .font(.system(size: 9, weight: .bold)).tracking(1.4).foregroundStyle(BondTheme.ink.opacity(0.4))
                }
