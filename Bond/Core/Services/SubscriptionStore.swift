    ...
    func loadProducts() async {
        ...
        do {
            ...
            productLoadFailure = yuklenen.isEmpty
                ? L10n.Paywall.productsFailed
                : nil
        } catch {
            products = []
            productLoadFailure = L10n.Paywall.productsFailed
        }
    }
        ...
        do {
            ...
            case .success(let verification):
                guard let islem = try? checkVerified(verification) else {
                    return .failed(L10n.Paywall.verifyFailed)
                }
                await islem.finish()
            ...
            case .pending:
                return .pending
            @unknown default:
                return .failed(L10n.Paywall.purchaseFailed)
            }
        } catch {
